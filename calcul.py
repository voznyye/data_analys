...
def przypiszKrotkomNumeryKlastrów():
    ...
    new_column = np.empty((len(dane_unpacked), 1))  
    for i in range(len(dane_unpacked)):
        ...
        for j, centroida in enumerate(Centroidy):
            ...
            if dist < minimum:
                minimum = dist
                minimumIndex = j
        new_column[i] = minimumIndex  
    dane_unpacked = np.hstack((dane_unpacked, new_column))  
    
def utwórzKlastry():
    global klastry
    dane_copy = dane_unpacked.copy() 
